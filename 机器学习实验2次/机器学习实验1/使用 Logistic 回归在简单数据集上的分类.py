...
def gradAscent(dataMatIn, classLabels,numIter=500):
    # 转化为矩阵[[1,1,2],[1,1,2]....]
    dataMatrix = mat(dataMatIn)             # 转换为 NumPy 矩阵，后面直接矩阵乘法
    ...
    for k in range(numIter):              #heavy on matrix operations
        ...
        # print 'hhhhhhh====', h
        # labelMat是实际值
        diff = (labelMat - h)              # 向量相减
        # 0.001* (3*m)*(m*1) 表示在每一个列上的一个误差情况，最后得出 x1,x2,xn的系数的偏移量
        weights = weights + alpha * dataMatrix.transpose() * diff # 矩阵乘法，最后得到回归系数
        thetas.append(array(weights))
    return thetas
...
def stocGradAscent0(dataMatrix, classLabels,numIter=500):
    ...
    for j in range(numIter):
        for i in range(m):
            ...
            diff = classLabels[i] - h
            # 0.01*(1*1)*(1*n)
            #print(weights, "*"*10 , dataMatrix[i], "*"*10 , error)
            weights = weights + alpha * diff * dataMatrix[i]
...
def stocGradAscent1(dataMatrix, classLabels, numIter=500):
    ...
    for j in range(numIter):
        ...
        for i in range(m):
            ...
            alpha = 4/(1.0+j+i)+0.001    # alpha 会随着迭代不断减小，但永远不会减小到0，因为后边还有一个常数项0.0001
            # 随机产生一个 0～len()之间的一个值
            # random.uniform(x, y) 方法将随机生成下一个实数，它在[x,y]范围内,x是这个范围内的最小值，y是这个范围内的最大值。
            randIndex = int(random.uniform(0,len(dataIndex)))
            # sum(dataMatrix[i]*weights)为了求 f(x)的值， f(x)=a1*x1+b2*x2+..+nn*xn
            h = sigmoid(sum(dataMatrix[dataIndex[randIndex]]*weights))
            diff = classLabels[dataIndex[randIndex]] - h
            # print weights, '__h=%s' % h, '__'*20, alpha, '__'*20, error, '__'*20, dataMatrix[randIndex]
            weights = weights + alpha * diff * dataMatrix[dataIndex[randIndex]]
            thetas.append(weights)
...
def testLR():
    ...
    # print dataArr
    thetas = gradAscent(dataArr, labelMat)
    # 数据可视化
    plotBestFit(dataArr, labelMat, thetas[-1])
    plotThetas(thetas)

    # thetas = stocGradAscent0(dataArr, labelMat)
    # plotBestFit(dataArr, labelMat, thetas[-1])
    # plotThetas(thetas)
    #
    # thetas = stocGradAscent1(dataArr, labelMat)
    # plotBestFit(dataArr, labelMat, thetas[-1])
    # plotThetas(thetas)
